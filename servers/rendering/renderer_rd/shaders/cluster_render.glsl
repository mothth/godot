...
layout(push_constant, std430) uniform Params {
	...
	// Most things have been moved here since we want parallel processing for portal clusters too,
	// just modifying the uniform buffer and going again isn't a great idea

	uint base_offset;	// Offset into element buffer
	uint dest_offset;	// Offset into render buffer
	...
	float inv_z_far;
	uvec2 cluster_screen_offset; // First portal cluster visible
}
params;
...
layout(set = 0, binding = 1) buffer restrict readonly RenderElements {
	RenderElement data[];
}
render_elements;
...
void main() {
	...
	vertex *= render_elements.data[element_index + params.base_offset].scale;

	vertex = vec4(vertex, 1.0) * render_elements.data[element_index + params.base_offset].transform_inv;
...
layout(set = 0, binding = 2, std430) buffer restrict ClusterRender {
	uint data[];
}
cluster_render;
...
void main() {
	...
	// (For portals, we use scissor testing so we don't accidentally go out of this range)
	uvec2 cluster = uvec2(gl_FragCoord.xy) >> params.screen_to_clusters_shift;
	cluster -= params.cluster_screen_offset;

	//get linear cluster offset from screen poss
	uint cluster_offset = cluster.x + params.cluster_screen_width * cluster.y;
	//multiply by data size to position at the beginning of the element list for this cluster
	cluster_offset = cluster_offset * params.cluster_data_size + params.dest_offset;
	...
	float unit_depth = depth_interp * params.inv_z_far;

	uint z_bit = clamp(uint(floor(unit_depth * 32.0)), 0, 31);

	uint z_write_offset = cluster_offset + params.cluster_depth_offset + element_index;
