...
REMOVE_MODULES = module_fbx_enabled=no module_multiplayer_enabled=no module_webrtc_enabled=no module_websocket_enabled=no
RELEASE_FLAGS = target=template_release production=yes arch=x86_64 opengl3=false disable_mobile=true builtin_openxr=no lto=full build_profile=$(PROFILE) $(REMOVE_MODULES)
...
build:
	scons platform=linuxbsd compiledb=yes opengl3=false disable_mobile=true debug_symbols=yes builtin_openxr=no
...
profiler:
	scons platform=linuxbsd compiledb=yes opengl3=false disable_mobile=true debug_symbols=yes builtin_openxr=no profiler=tracy profiler_path=$(TRACY_PATH)
	mv $(EXPORT_TEMPLATES)/$(VERSION)/linux_release.x86_64 $(EXPORT_TEMPLATES)/$(VERSION)/old.linux_release.x86_64
	mv $(EXPORT_TEMPLATES)/$(VERSION)/windows_release.x86_64.exe $(EXPORT_TEMPLATES)/$(VERSION)/old.windows_release.x86_64.exe
...
compile_commands.json:
	scons compiledb=yes opengl3=false disable_mobile=true build_profile=$(PROFILE) $(REMOVE_MODULES) compile_commands.json
